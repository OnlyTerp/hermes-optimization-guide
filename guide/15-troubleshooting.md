...
| Compression runs every turn and barely shrinks anything | The session trigger was lowered to fit a small auxiliary compression model's window | Use a compression model whose window is at least the trigger (`compression.threshold_tokens`, 256K by default) | [Issue #53008](https://github.com/NousResearch/hermes-agent/issues/53008), [Configuration](https://hermes-agent.nousresearch.com/docs/user-guide/configuration#context-compression) |

## Messaging gateway

| Symptom | Cause | Fix | Source |
|---|---|---|---|
| The bot dies when you close SSH, or doesn't come back after a reboot | User-level systemd units stop at logout unless lingering is enabled | Run `hermes gateway install`, which tries to enable lingering; otherwise `sudo loginctl enable-linger $USER`. Or use a boot-time system unit: `sudo hermes gateway install --system --run-as-user <user>`. | [Messaging](https://hermes-agent.nousresearch.com/docs/user-guide/messaging) |
| Restart loop ("Gateway already running", restart spam) | A custom `ExecStopPost=… kill -9` drop-in, or a restart race | Remove the drop-in (`systemctl --user edit hermes-gateway`, delete the line, `daemon-reload`). Restart with `hermes gateway restart`, which drains first. | [Messaging](https://hermes-agent.nousresearch.com/docs/user-guide/messaging), [issue #21915](https://github.com/NousResearch/hermes-agent/issues/21915) |
...
| Some gateways (WeCom/Weixin) don't inject `USER.md`/`MEMORY.md` | [#96134](https://github.com/NousResearch/hermes-agent/issues/96134) |
...
| Desktop on WSL2: gateway exits every ~2 minutes | [#95189](https://github.com/NousResearch/hermes-agent/issues/95189) |
