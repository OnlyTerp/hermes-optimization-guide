...
## Agent Tooling Updates (v0.20 era, August 2026)

- **Claude Code**: still the best premium worker lane for high-stakes PRs — `--allowedTools` scoping, JSON output capture, and a stable print mode are what the delegation skills rely on; verify versions with `--version` before promoting.
- **Codex**: pin a stable line for unattended Kanban lanes and verify with `codex --version` — alpha lines have broken unattended flows. Point it at `hermes proxy` when you want ChatGPT/Codex OAuth without another API key.
- **Gemini**: the Gemini-CLI OAuth providers were **removed from Hermes** — use a `GEMINI_API_KEY` or Vertex AI ([Part 9](./part9-custom-models.md)); the standalone CLI still works as a delegated reader lane.
- **OpenCode**: the provider surface moved — dropped its old OpenCode-Zen hosting mid-2026, added the keyless **OpenCode Free** tier; re-check your model config after upgrading. Useful with Kimi K3 / GLM budget lanes.
...
Two notes still worth their own lines in the v0.20 era:
